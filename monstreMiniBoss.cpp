#include "MonstreMiniBoss.h"
#include <iostream>
#include "ActionACT.h"

MonstreMiniBoss::MonstreMiniBoss(string nom, int hp, int attaque, int defense, int mercyObjectif)
    : Monstre(nom, "MINIBOSS", hp, attaque, defense, mercyObjectif, 30)  {}
    
    int MonstreMiniBoss::addAction(ActionACT* a) {
    if (actions.size() < 3) {
        actions.push_back(a);
        return 1;
    }
    cout << "Impossible d'ajouter l'action, le Monstre Mini-Boss a deja ses 3 actions !" << endl;
    return 0;
    }
    
    void MonstreMiniBoss::afficherMonstre() const {
    cout << "--- " << nom << " (" << categorie << ") ---" << endl;
    cout << "HP : " << hp << " | Attaque : " << attaque << " | Defense : " << defense << endl;
    cout << "Mercy : " << mercyActuel << "/" << mercyObjectif << endl;
    cout << "Statut actuel : " << (statut.empty() ? "En combat" : statut) << endl;
    
    cout << "Actions disponibles (" << actions.size() << "/3) :" << endl;
    if (actions.empty()) {
        cout << "  (Aucune action pour le moment)" << endl;
    } else {
        // On parcourt la liste des actions
        for (size_t i = 0; i < actions.size(); i++) {
            cout << "  - " << actions[i]->getIdentifiant() << endl;
        }
    }
    cout << "-----------------------" << endl;
    }

    const std::vector<ActionACT*>& MonstreMiniBoss::getActions() const {
        return actions;
    }